import numpy as np
import numpy.testing as npt
import pytest

from inflammation.models import daily_mean, daily_max, daily_min


def test_everything_works():
    npt.assert_array_equal(np.array([0, 0]), np.array([0, 0]))


def test_daily_mean_zeros():
    """Test that mean function works for an array of positive arrays"""

    test_array = np.array([[1, 2], [3, 4], [5, 6]])

    # Need to use numpy testing
    npt.assert_array_equal(np.array([3, 4]), daily_mean(test_array))


def test_daily_min():
    """Test that min function works for an array of positive arrays."""

    test_array = np.array([[1, 2], [3, 4]])

    npt.assert_array_equal(np.array([1, 2]), daily_min(test_array))


def test_daily_max():

    test_array = np.array([[1, 2], [3, 4]])

    npt.assert_array_equal(np.array([3, 4]), daily_max(test_array))


def test_daily_min_string():
    """Test for TypeError when passing strings."""

    from pytest import raises

    with raises(TypeError):
        daily_min([['Cannot', 'min'], ['string', 'arguments']])


def test_daily_mean_bool():
    """Test for TypeError when passing bool."""

    from pytest import raises

    with raises(TypeError):
        daily_mean([1, 3], axis=True)


@pytest.mark.parametrize(
    "test, expected",
    [
        ([[0, 0], [0, 0], [0, 0]], [0, 0]),
        ([[1, 2], [3, 4], [5, 6]], [3, 4]),
    ])
def test_daily_mean(test, expected):
    """Test mean function works for array of zeroes and positive integers."""
    from inflammation.models import daily_mean
    npt.assert_array_equal(np.array(expected), daily_mean(np.array(test)))
